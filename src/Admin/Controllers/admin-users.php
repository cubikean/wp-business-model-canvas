            ...
            <table class="wp-list-table widefat striped" id="supervisors-table">
                ...
                <tbody>
                    <?php foreach ($supervisors as $supervisor): ?>
                        ...
                        $can_access_backoffice = WP_BMC_Auth::can_access_backoffice($supervisor->ID);
                        ...
                        <tr class="supervisor-row" data-user-id="<?php echo $supervisor->ID; ?>">
                            ...
                            <td class="supervisor-access">
                                <span class="access-badge <?php echo $can_access_backoffice ? 'access-allowed' : 'access-blocked'; ?>" data-supervisor-id="<?php echo $supervisor->ID; ?>">
                                    <?php echo $can_access_backoffice ? 'Autorisé' : 'Bloqué'; ?>
                                </span>
                            </td>
                            <td class="supervisor-actions">
                                <div class="action-buttons">
                                    ...
                                    <?php 
                                    $toggle_disabled = ($supervisor->ID == $current_user_id);
                                    ?>

                                    <?php if ($toggle_disabled): ?>
                                        <span class="current-user-badge" title="Vous ne pouvez pas modifier votre propre accès">
                                            <i class="fas fa-user-lock"></i> Accès propre
                                        </span>
                                    <?php else: ?>
                                        <button class="button button-small button-secondary toggle-backoffice-access <?php echo $can_access_backoffice ? 'access-allowed' : 'access-blocked'; ?>"
                                            data-supervisor-id="<?php echo $supervisor->ID; ?>"
                        data-allow="<?php echo $can_access_backoffice ? '1' : '0'; ?>"
                                            title="<?php echo $can_access_backoffice ? 'Bloquer l\'accès backoffice' : 'Autoriser l\'accès backoffice'; ?>">
                                            <i class="fas <?php echo $can_access_backoffice ? 'fa-unlock' : 'fa-lock'; ?>"></i>
                                            <?php echo $can_access_backoffice ? 'Accès autorisé' : 'Accès bloqué'; ?>
                                        </button>
                                    <?php endif; ?>
