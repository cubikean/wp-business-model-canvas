...
jQuery(document).ready(function($) {
    ...
    $(document).on('click', '.mark-all-read-btn', function() {
        ...
        Promise.all(promises).then(function(responses) {
            ...
            if (successCount > 0) {
                ...
                showMessage(successCount + ' notification(s) marquée(s) comme lue(s).', 'success');
            } else {
                showMessage('Erreur lors du marquage des notifications.', 'error');
            }
        }).catch(function() {
            showMessage('Erreur lors du marquage des notifications.', 'error');
        }).finally(function() {
            $btn.prop('disabled', false).html('<i class="fas fa-check-double"></i> Tout marquer comme lu');
        });
    });
    ...
    $(document).on('click', '.grade-btn', function() {
        ...
        var url = window.location.origin + '/business-model-canvas/?admin_view=true&user_id=' + userId + '&project_id=' + projectId;
    ...
    $(document).on('click', '.rate-brick-btn', function() {
        openGradingModal($(this).data('project-id'), $(this).data('section'), $(this).data('section-title'));
    });
    ...
    function showMessage(message, type) {
        var $message = $('<div class="notice notice-' + type + ' is-dismissible wp-bmc-message"><p>' + message + '</p></div>');
    ...
    function openGradingModal(projectId, section, sectionTitle) {
        var modal = $('<div class="wp-bmc-popup grading-modal">' +
            ...
            '<div class="popup-content">' +
                ...
                '</div>' +
                '<div class="popup-body">' +
                    '<form id="grading-form" data-project-id="' + projectId + '">' +
