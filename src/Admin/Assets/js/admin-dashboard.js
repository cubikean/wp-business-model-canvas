...
jQuery(document).ready(function ($) {
  ...
  $(document).on("wp_bmc:admin_section_saved", function (_event, details) {
    if (
      !details ||
      !details.projectId ||
      !details.section ||
    ...
    openGradingModal(details.projectId, details.section, details.sectionTitle, {
      force: true,
    });
  });
  ...
  // Fonction pour mettre à jour la section des demandes de notation
  function updateGradingRequestsSection() {
    // Vérifier si on est sur la page dashboard admin
    if (!$('.wp-bmc-notifications-section').length) {
      return;
    }

    // Vérifier si wp_bmc_admin_ajax est disponible (priorité au nonce admin)
    if (typeof wp_bmc_admin_ajax === 'undefined') {
      // Essayer avec wp_bmc_ajax si disponible
      if (typeof wp_bmc_ajax === 'undefined') {
        return;
      }
      var ajaxUrl = wp_bmc_ajax.ajax_url;
      var nonce = wp_bmc_ajax.nonce;
    } else {
      var ajaxUrl = wp_bmc_admin_ajax.ajax_url;
      var nonce = wp_bmc_admin_ajax.nonce;
    }

    // Récupérer les demandes de notation en attente
    $.post(ajaxUrl, {
      action: 'wp_bmc_get_admin_pending_grading_count',
      nonce: nonce
    }, function(response) {
      if (response.success) {
        ...
        var $notificationsSection = $('.wp-bmc-notifications-section');
        var $existingGradingSection = $notificationsSection.find('.grading-requests-section');

        if (count > 0 && html) {
          // Mettre à jour ou créer la section
          if ($existingGradingSection.length > 0) {
            // Remplacer la section existante
            $existingGradingSection.fadeOut(200, function() {
              $(this).replaceWith(html);
              $notificationsSection.find('.grading-requests-section').hide().fadeIn(200);
            });
          } else {
            // Ajouter la nouvelle section
            $notificationsSection.append(html);
            $notificationsSection.find('.grading-requests-section').hide().fadeIn(200);
          }
        } else {
          // Supprimer la section si aucune demande
          if ($existingGradingSection.length > 0) {
            $existingGradingSection.fadeOut(300, function() {
              $(this).remove();
            });
          }
        }
  ...
  function openGradingModal(projectId, section, sectionTitle, options) {
    
    var settings = $.extend(
      {
        force: false,
        forceMessage:
          "Veuillez attribuer une note avant de fermer cette fenêtre.",
      },
      options || {}
    );
    ...
    if (settings.force) {
      modal.addClass("force-grading");
      modal.find(".popup-close").remove();
      modal
        .find(".popup-body")
        .prepend(
          '<div class="grading-force-message"><i class="fas fa-exclamation-triangle"></i> ' +
            settings.forceMessage +
            "</div>"
        );
    }
    ...
    modal.find("#grading-form").on("submit", function (e) {
      ...
      $.post(wp_bmc_admin_ajax.ajax_url, formData, function (response) {
        if (response.success) {
          ...
          // Mettre à jour toute la section des demandes de notation
          setTimeout(function() {
            updateGradingRequestsSection();
          }, 500);
    ...
    if (settings.force) {
      modal.find(".popup-overlay").on("click", function () {
        WP_BMC_Toast.warning(settings.forceMessage);
      });
    } else {
  ...
  $(document).on("keydown", function (e) {
    ...
    if (e.key === "Escape") {
      var $forcedModal = $(".wp-bmc-popup.force-grading:visible");
      if ($forcedModal.length) {
        WP_BMC_Toast.warning(
          "Veuillez attribuer une note avant de fermer cette fenêtre."
        );
        return;
      }
  ...
  // ========================================
  // MISE À JOUR AUTOMATIQUE DES DEMANDES DE NOTATION
  // ========================================
  
  // Charger la section des demandes au chargement de la page
  updateGradingRequestsSection();
  
  // Mettre à jour toutes les 30 secondes
  setInterval(updateGradingRequestsSection, 30000);
});
