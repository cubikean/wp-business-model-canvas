...
jQuery(document).ready(function($) {
    ...
    // Activer/Désactiver l'accès backoffice d'un superviseur
    $(document).on('click', '.toggle-backoffice-access', function() {
        var $btn = $(this);
        var supervisorId = $btn.data('supervisor-id');
        var currentAllow = $btn.data('allow') === 1 || $btn.data('allow') === '1';
        var newAllow = currentAllow ? '0' : '1';

        var confirmMessage = newAllow === '1'
            ? 'Autoriser ce chargé d\'accompagnement à accéder au backoffice WordPress ?'
            : 'Bloquer l\'accès backoffice pour ce chargé d\'accompagnement ?';
        ...
        $.post(wp_bmc_admin_ajax.ajax_url, {
            action: 'wp_bmc_toggle_supervisor_access',
            supervisor_id: supervisorId,
            allow_admin: newAllow,
            nonce: wp_bmc_admin_ajax.nonce
        }, function(response) {
            if (response.success) {
                WP_BMC_Toast.success(response.data.message);

                var allowAdmin = response.data.allow_admin === true || response.data.allow_admin === '1';
                var newLabel = allowAdmin ? 'Accès autorisé' : 'Accès bloqué';
                var newIcon = allowAdmin ? 'fa-unlock' : 'fa-lock';
                var newTitle = allowAdmin ? 'Bloquer l\'accès backoffice' : 'Autoriser l\'accès backoffice';

                $btn
                    .data('allow', allowAdmin ? '1' : '0')
                    .removeClass('access-allowed access-blocked')
                    .addClass(allowAdmin ? 'access-allowed' : 'access-blocked')
                    .html('<i class="fas ' + newIcon + '"></i> ' + newLabel)
                    .attr('title', newTitle);

                var $badge = $('.access-badge[data-supervisor-id="' + supervisorId + '"]');
                if ($badge.length) {
                    $badge
                        .toggleClass('access-allowed', allowAdmin)
                        .toggleClass('access-blocked', !allowAdmin)
                        .text(allowAdmin ? 'Autorisé' : 'Bloqué');
                }
            } else {
                WP_BMC_Toast.error(response.data || 'Erreur lors de la mise à jour de l\'accès backoffice.');
                $btn.html(originalHtml);
                $btn.prop('disabled', false);
            }
        }).fail(function() {
            WP_BMC_Toast.error('Erreur de connexion lors de la mise à jour.');
            $btn.html(originalHtml);
            $btn.prop('disabled', false);
        }).always(function() {
            $btn.prop('disabled', false);
