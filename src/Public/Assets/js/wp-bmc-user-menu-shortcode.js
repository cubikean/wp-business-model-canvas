...
jQuery(document).ready(function($) {
    ...
    // Fonction pour charger et mettre à jour le nombre de demandes de notation
    function updateGradingBadgeCount() {
        // Vérifier si wp_bmc_ajax est disponible
        if (typeof wp_bmc_ajax === 'undefined') {
            return;
        }
        ...
        const userMenu = $('.wp-bmc-user-menu-shortcode');
        ...
        $.post(wp_bmc_ajax.ajax_url, {
            ...
            if (response.success && response.data.count > 0) {
                ...
                $('.wp-bmc-grading-badge').each(function() {
                    const $badge = $(this);
                    $badge.text(response.data.count).show();
                });
            } else {
                // Masquer le badge si aucune demande
                $('.wp-bmc-grading-badge').hide();
            }
        }).fail(function() {
    ...
    // Charger le nombre de demandes au chargement de la page
    updateGradingBadgeCount();
    
    // Mettre à jour toutes les 30 secondes
    setInterval(updateGradingBadgeCount, 30000);
    ...
    window.WP_BMC_UserMenuShortcode = {
        ...
        updateGradingBadgeCount: updateGradingBadgeCount
    };
});
