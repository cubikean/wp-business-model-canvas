...
class WP_BMC_Auth {
    ...
    public function __construct() {
        ...
        add_action('admin_init', array($this, 'enforce_supervisor_admin_access'));
    }
    ...
    /**
     * Vérifier si un utilisateur administrateur peut accéder au backoffice WordPress
     *
     * @param int|null $user_id
     * @return bool
     */
    public static function can_access_backoffice($user_id = null) {
        if (null === $user_id) {
        ...
        if (!user_can($user_id, 'manage_options')) {
            // Les utilisateurs non administrateurs ne sont pas concernés par cette restriction
            return true;
        }

        $meta = get_user_meta($user_id, 'wp_bmc_allow_backoffice', true);

        if ($meta === '' || $meta === null) {
            // Par défaut, l'accès est autorisé
            return true;
        }

        return in_array($meta, array('1', 1, true, 'true'), true);
    }

    /**
     * Empêcher les chargés d'accompagnement bloqués d'accéder au backoffice
     */
    public function enforce_supervisor_admin_access() {
        if (!is_admin()) {
            return;
        }

        if (wp_doing_ajax()) {
            // Ne pas bloquer les appels AJAX (utiles côté public)
            return;
        }

        // Autoriser l'écran de connexion WordPress
        ...
        // Les administrateurs doivent disposer explicitement de l'accès
        if (current_user_can('manage_options')) {
            if (!self::can_access_backoffice($current_user_id)) {
                error_log('WP_BMC_Auth::enforce_supervisor_admin_access() - Accès backoffice bloqué pour l\'utilisateur ' . $current_user_id);
        ...
        // Tous les autres utilisateurs sont systématiquement redirigés
        wp_redirect(home_url('/dashboard/'));
        exit;
    }
