...
// Handler pour activer/désactiver l'accès backoffice d'un superviseur
add_action('wp_ajax_wp_bmc_toggle_supervisor_access', 'wp_bmc_toggle_supervisor_access_handler');
function wp_bmc_toggle_supervisor_access_handler() {
    check_ajax_referer('wp_bmc_admin_nonce', 'nonce');
    ...
    $allow_admin = isset($_POST['allow_admin']) && $_POST['allow_admin'] === '1';
    ...
    if ($supervisor_id === get_current_user_id()) {
        wp_send_json_error('Vous ne pouvez pas modifier votre propre accès backoffice.');
    }

    $user = get_user_by('ID', $supervisor_id);
    if (!$user || !user_can($user, 'manage_options')) {
        wp_send_json_error('Superviseur introuvable ou non administrateur.');
    }

    update_user_meta($supervisor_id, 'wp_bmc_allow_backoffice', $allow_admin ? '1' : '0');

    $message = $allow_admin
        ? 'Accès backoffice rétabli pour ce chargé d\'accompagnement.'
        : 'Accès backoffice désactivé pour ce chargé d\'accompagnement.';
...
function wp_bmc_save_canvas_handler() {
    ...
    if (!is_array($canvas_data)) {
        wp_send_json_error('Format des données du canvas invalide.');
    }
    ...
    if (!$project_id) {
        $user = WP_BMC_Auth::get_current_user();
        $projects = $user ? WP_BMC_Database::get_user_projects($user->user_id) : array();
        
        // Les admins doivent fournir explicitement l'ID de projet pour éviter d'écraser un autre canvas
        if (current_user_can('manage_options')) {
            wp_send_json_error('ID de projet manquant pour la sauvegarde.');
        }
    ...
    // Gestion des versions côté client pour éviter les écrasements concurrents
    $canvas_versions = array();
    if (isset($_POST['canvas_versions']) && is_array($_POST['canvas_versions'])) {
        $canvas_versions = array_map('sanitize_text_field', $_POST['canvas_versions']);
    }
    
    // Utiliser un filtre personnalisé pour autoriser les images et le contenu enrichi
    // Utiliser wp_kses_post comme base puis ajouter nos règles spécifiques
    ...
    // S'assurer que img est bien configuré avec tous ses attributs
    ...
    // Récupérer le contenu actuel du canvas (avec meta) pour comparer les changements
    $current_canvas_data = WP_BMC_Database::get_canvas_data($project_id, true);
    
    // Première passe : préparer les sections et détecter d'éventuels conflits
    foreach ($canvas_data as $section => $raw_content) {
        $section = sanitize_text_field($section);
        
        // Nettoyer les slashes d'échappement avant le traitement
        $raw_content = stripslashes($raw_content);
        
        // Appliquer le filtre de sécurité en préservant les images
        $content = wp_kses($raw_content, $allowed_html);
        
        // Sécurité supplémentaire : vérifier et normaliser les URLs des images
        $content = preg_replace_callback('/<img([^>]*)src=["\']([^"\']+)["\']([^>]*)>/i', function($matches) use ($project_id) {
            $url = $matches[2];
            $home_url = home_url();
            ...
            // Autoriser :
            // 1. URLs absolues du site (avec ou sans slash final)
            // 2. URLs relatives commençant par /wp-content/uploads/
            // 3. Data URIs pour les images inline
            // 4. URLs contenant wp-bmc-files (fichiers uploadés via le plugin)
            $is_valid = (
                strpos($url, $home_url) === 0 || 
                strpos($url, $home_url_no_slash) === 0 ||
                strpos($url, 'data:image') === 0 ||
                strpos($url, 'wp-bmc-files') !== false
            );
            
            if ($is_valid) {
                // Encoder correctement l'URL sans double encodage
                $safe_url = esc_url($url);
                return '<img' . $matches[1] . 'src="' . $safe_url . '"' . $matches[3] . '>';
            }
            
            error_log("wp_bmc_save_canvas_handler - URL d'image rejetée: " . $url);
            return '';
        }, $content);
        ...
        if ($raw_content !== $content) {
            ...
            error_log("  Avant: " . substr($raw_content, 0, 5000) . (strlen($raw_content) > 5000 ? '...' : ''));
            error_log("  Après: " . substr($content, 0, 5000) . (strlen($content) > 5000 ? '...' : ''));
            
            // Log spécifique pour les images
            if (strpos($raw_content, '<img') !== false) {
                error_log("  ATTENTION: Le contenu contenait des balises <img>");
                if (strpos($content, '<img') === false) {
                    error_log("  ERREUR: Les balises <img> ont été supprimées par le filtre !");
                } else {
                    error_log("  OK: Les balises <img> ont été préservées");
                }
            }
        }
        
        // Vérifier si le contenu a réellement changé par rapport à ce qui est en base
        $current_entry = isset($current_canvas_data[$section]) ? $current_canvas_data[$section] : array('content' => '', 'updated_at' => null);
        ...
        $client_version = isset($canvas_versions[$section]) ? $canvas_versions[$section] : null;
        $server_version = isset($current_entry['updated_at']) ? $current_entry['updated_at'] : null;
        
        // Détection de conflit : le client édite une version obsolète
        if ($client_version && $server_version && $client_version !== $server_version && $content_has_changed) {
            $conflicts[] = array(
                'section' => $section,
        ...
        $prepared_sections[$section] = array(
            'content' => $content,
            'content_has_changed' => $content_has_changed,
            'expected_version' => $server_version ?: $client_version,
            'server_version' => $server_version
        );
    }
    
    // Si conflit détecté, on bloque la sauvegarde pour éviter d'écraser une modification récente
    if (!empty($conflicts)) {
        wp_send_json_error(array(
            'message' => 'Conflit détecté : le contenu a été modifié par un autre utilisateur. Veuillez recharger avant de réessayer.',
            'error_type' => 'conflict',
            'conflicts' => $conflicts
        ));
    }
    
    // Deuxième passe : sauvegarder uniquement si pas de conflit détecté
    foreach ($prepared_sections as $section => $section_data) {
        $save_result = WP_BMC_Database::save_canvas_data($project_id, $section, $section_data['content'], $section_data['expected_version']);
        
        // Gestion des conflits survenus entre la préparation et l'écriture
        if (is_array($save_result) && !empty($save_result['conflict'])) {
            $conflicts[] = array(
                'section' => $section,
                'client_version' => $section_data['expected_version'],
                'server_version' => isset($save_result['current_updated_at']) ? $save_result['current_updated_at'] : $section_data['server_version'],
                'server_content' => isset($save_result['current_content']) ? $save_result['current_content'] : $section_data['content']
            );
            continue;
        }
        
        if ((is_array($save_result) && !empty($save_result['success'])) || $save_result === true) {
            $success_count++;
            
            if (is_array($save_result) && isset($save_result['updated_at'])) {
                $updated_versions[$section] = $save_result['updated_at'];
            }
            
            // MODE 2 : Créer une révision si un admin modifie le contenu ET que le contenu a réellement changé
            if (WP_BMC_REVISION_ON_ADMIN_EDIT && $is_admin && $section_data['content_has_changed']) {
            ...
            } elseif (WP_BMC_REVISION_ON_ADMIN_EDIT && $is_admin && !$section_data['content_has_changed']) {
                error_log("wp_bmc_save_canvas_handler - Pas de révision créée pour section '$section' : contenu inchangé");
            }
        }
    }
    
    // Si des conflits sont survenus pendant l'écriture, informer le client
    if (!empty($conflicts)) {
        wp_send_json_error(array(
            'message' => 'Certaines sections n\'ont pas été sauvegardées car elles ont été mises à jour par un autre utilisateur.',
            'error_type' => 'conflict',
            'conflicts' => $conflicts,
...
function wp_bmc_get_canvas_handler() {
    // Accepter le nonce admin ou public comme pour la sauvegarde
    $nonce_valid = false;
    
    if (isset($_POST['nonce']) && wp_verify_nonce($_POST['nonce'], 'wp_bmc_admin_nonce')) {
        $nonce_valid = true;
        error_log('wp_bmc_get_canvas_handler - Nonce admin validé');
    } elseif (isset($_POST['nonce']) && wp_verify_nonce($_POST['nonce'], 'wp_bmc_nonce')) {
        $nonce_valid = true;
        error_log('wp_bmc_get_canvas_handler - Nonce public validé');
    }
    
    if (!$nonce_valid) {
        error_log('wp_bmc_get_canvas_handler - Nonce invalide: ' . (isset($_POST['nonce']) ? $_POST['nonce'] : 'nonce manquant'));
        wp_send_json_error('Nonce de sécurité invalide.');
    }
    ...
    $canvas_data_with_meta = WP_BMC_Database::get_canvas_data($project_id, true);
    
    $canvas_data = array();
    $canvas_versions = array();
    
    foreach ($canvas_data_with_meta as $section => $entry) {
        $canvas_data[$section] = isset($entry['content']) ? $entry['content'] : '';
        $canvas_versions[$section] = isset($entry['updated_at']) ? $entry['updated_at'] : null;
    }
...
function wp_bmc_save_section_rating_handler() {
    ...
    if ($result) {
        ...
        // Marquer la demande de notation comme notée
        WP_BMC_Database::mark_grading_request_as_graded($project_id, $section);
        error_log("wp_bmc_save_section_rating_handler - Demande de notation marquée comme notée pour section '$section'");
        
        // Envoyer un email de notification aux utilisateurs du projet
        $project = WP_BMC_Database::get_project($project_id);
        if ($project) {
            // Récupérer les utilisateurs du projet (v2.0 utilise la table de liaison)
            $project_users = WP_BMC_Database::get_project_users($project_id);
            
            if (!empty($project_users)) {
                // Récupérer le nom de l'admin
                $admin = get_userdata($admin_id);
                $admin_name = $admin ? trim($admin->first_name . ' ' . $admin->last_name) : 'Votre chargé d\'accompagnement';
                if (empty(trim($admin_name))) {
                    $admin_name = $admin->display_name ?: 'Votre chargé d\'accompagnement';
                }
                ...
                $canvas_sections = wp_bmc_get_canvas_sections('global', true);
                $section_title = isset($canvas_sections[$section]) ? $canvas_sections[$section]['title'] : $section;
                
                // Vérifier que la fonction d'envoi d'email existe
                if (!function_exists('wp_bmc_send_section_graded_notification')) {
                    include_once WP_BMC_PLUGIN_DIR . 'src/Shared/Utils/email-templates.php';
                }
                
                // Nettoyer les données pour éviter les problèmes d'échappement
                $project_title = stripslashes($project->title);
                $section_title = stripslashes($section_title);
                $comment = stripslashes($comment);
                $admin_name = stripslashes($admin_name);
                
                // Envoyer un email à chaque utilisateur du projet
                foreach ($project_users as $user) {
                    if (!empty($user->email)) {
                        ...
                        $email_sent = wp_bmc_send_section_graded_notification(
                            $user->email,
                            $user_first_name,
                            $user_last_name,
                            $section_title,
                        ...
                        if ($email_sent) {
                            error_log("wp_bmc_save_section_rating_handler - Email de notification envoyé avec succès à {$user->email}");
                        } else {
                            error_log("wp_bmc_save_section_rating_handler - ERREUR: Échec de l'envoi de l'email à {$user->email}");
                        }
                    }
                }
            } else {
                error_log("wp_bmc_save_section_rating_handler - ERREUR: Aucun utilisateur trouvé pour project_id $project_id");
            }
        } else {
            error_log("wp_bmc_save_section_rating_handler - ERREUR: Projet non trouvé pour project_id $project_id");
        }
...
// Handler pour obtenir le nombre de demandes de notation en attente pour un admin
add_action('wp_ajax_wp_bmc_get_admin_pending_grading_count', 'wp_bmc_get_admin_pending_grading_count_handler');
add_action('wp_ajax_nopriv_wp_bmc_get_admin_pending_grading_count', 'wp_bmc_get_admin_pending_grading_count_handler');
function wp_bmc_get_admin_pending_grading_count_handler() {
    // Vérifier le nonce admin ou public selon le contexte
    $nonce_valid = false;
    ...
    if (!$current_user->is_admin && !current_user_can('manage_options')) {
        wp_send_json_success(array('count' => 0, 'html' => ''));
        return;
    }
    
    $admin_id = get_current_user_id();
    $pending_requests = WP_BMC_Database::get_pending_grading_requests($admin_id);
    $count = count($pending_requests);
    
    // Générer le HTML de la section des demandes
    $html = '';
        ...
        <div class="grading-requests-section">
            <div class="grading-requests-header">
                <h3>
                    ...
                    <span class="grading-count"><?php echo $count; ?></span>
                </h3>
            </div>
                        ...
                        <div class="request-content">
                            <div class="request-info">
                                ...
                                <div class="request-details">
                                    demande un avis pour la section
...
add_action('wp_ajax_wp_bmc_check_grading_request_admin', 'wp_bmc_check_grading_request_handler');
function wp_bmc_check_grading_request_handler() {
    // Vérifier le nonce selon le contexte (admin ou public)
    $is_admin_action = isset($_POST['action']) && $_POST['action'] === 'wp_bmc_check_grading_request_admin';
    if ($is_admin_action) {
        check_ajax_referer('wp_bmc_admin_nonce', 'nonce');
        if (!current_user_can('manage_options')) {
            wp_send_json_error('Accès réservé aux administrateurs.');
    ...
    // Si un project_id est fourni (mode admin), l'utiliser
    if ($project_id) {
        $has_pending_request = WP_BMC_Database::has_pending_grading_request($project_id, $section);
    } else {
        // Sinon, récupérer le projet de l'utilisateur
        $user = WP_BMC_Auth::get_current_user();
        $projects = WP_BMC_Database::get_user_projects($user->user_id);
...
function wp_bmc_request_grading_handler() {
    ...
    if (!WP_BMC_Auth::is_logged_in()) {
        wp_send_json_error('Vous devez être connecté pour demander un avis.');
    }
    
    if (!isset($_POST['section'])) {
        wp_send_json_error('Section manquante pour la demande d\'avis.');
    }
    
    $section = sanitize_text_field($_POST['section']);
    $section_title = isset($_POST['section_title']) ? sanitize_text_field(stripslashes($_POST['section_title'])) : $section;
    ...
    if ($has_pending_request) {
        wp_send_json_error('Une demande d\'avis est déjà en attente pour cette section.');
        return;
    }
    ...
    if ($result) {
        ...
        wp_send_json_error('Erreur lors de l\'envoi de la demande d\'avis.');
    }
}
...
function wp_bmc_notify_admins_grading_request($project, $section, $section_title, $user) {
    ...
    $message = sprintf(
        'Un étudiant (%s) demande un avis pour la section "%s" de son projet "%s".',
        $user_name,
        $section_title,
        $project->title
    );
    ...
    $subject = 'Demande d\'avis - ' . $project->title;
...
function wp_bmc_export_all_data_handler() {
    ...
    // S'assurer que $project_todos est toujours un array
    if (!is_array($project_todos)) {
        $project_todos = array();
    }
    ...
    foreach ($sections_config as $section_key => $section_config) {
        ...
        $section_ratings = is_array($project_ratings) ? array_filter($project_ratings, function($rating) use ($section_key) {
    ...
    $total_todos = is_array($project_todos) ? count($project_todos) : 0;
    $total_completed = is_array($project_todos) ? count(array_filter($project_todos, function($todo) {
        return intval($todo->is_completed) === 1;
    })) : 0;
...
function wp_bmc_generate_pdf_gotenberg_handler() {
    ...
    error_log('PDF: Ratings count: ' . (is_array($project_ratings) ? count($project_ratings) : 0));
    ...
    foreach ($sections_config as $section_key => $section_config) {
        ...
        $section_todos = is_array($project_todos) ? array_filter($project_todos, function($todo) use ($section_key) {
            return $todo->section === $section_key;
        }) : array();
    ...
    $gotenberg_url = 'https://gotenberg.com';
...
// Handler pour définir l'ID Pepitizy d'un projet
add_action('wp_ajax_wp_bmc_set_project_pepitizy_id', 'wp_bmc_set_project_pepitizy_id_handler');
function wp_bmc_set_project_pepitizy_id_handler() {
    check_ajax_referer('wp_bmc_admin_nonce', 'nonce');
    ...
    $project_id = intval($_POST['project_id']);
    $pepitizy_id = isset($_POST['pepitizy_id']) ? sanitize_text_field($_POST['pepitizy_id']) : '';
    ...
    // Sauvegarder l'ID Pepitizy
    $result = WP_BMC_Database::update_project_pepitizy_id($project_id, $pepitizy_id);
    
    if ($result) {
        wp_send_json_success(array(
            'message' => 'ID Pepitizy enregistré avec succès',
            'pepitizy_id' => $pepitizy_id
        ));
    } else {
        wp_send_json_error('Erreur lors de l\'enregistrement de l\'ID Pepitizy');
    }
}
